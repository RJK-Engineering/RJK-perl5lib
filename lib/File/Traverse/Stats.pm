...
use File::File;
use File::Visitor;
use File::TraverseStats;

sub new {
    my $self = bless {}, shift;
    $self->{visitor} = &File::Visitor::getVisitor || new File::Visitor;
    $self->{stats} = new File::TraverseStats();
...
sub traverse {
    my $self = shift;
    my $file = &File::File::getFile || die "Insufficient arguments";
    my $visitor = &File::Visitor::getVisitor || $self->{visitor};
...
sub readdir {
    ...
    my @entries = grep {
        $_ ne $File::File::curdir && $_ ne $File::File::updir &&
