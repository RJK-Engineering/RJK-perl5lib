...
sub files {
    ...
    #~ my @files = grep { -f $_->{path} && $_ } map { new RJK::IO::File($self->{dir}, $_) } readdir $dh;
    my @files = grep { -f $_->{path} } map { new RJK::IO::File($self->{path}, $_) } readdir $dh;
    closedir $dh;
...
sub children {
    my $self = shift;
    opendir my $dh, $self->{path};
    my @files = map { new RJK::IO::File($self->{dir}, $_) } readdir $dh;
    closedir $dh;
    return wantarray ? @files : \@files;
}
