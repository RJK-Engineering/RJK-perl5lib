package RJK::File::Visitor;

use strict;
use warnings;

use RJK::File::Visitor::Wrapper;
use RJK::Scalar::Blessed::Util qw(is_of_type);
...
sub getVisitor {
    ...
    if (ref $opts{visitor}) {
        if (is_of_type RJK::File::Visitor, $opts{visitor}) {
            return $opts{visitor};
        } else {
            die "Not a RJK::File::Visitor";
        }
    }
    ...
    if ($opts{preVisitDir} || $opts{postVisitDir}  ||
        ...
        return new RJK::File::Visitor::Wrapper(
