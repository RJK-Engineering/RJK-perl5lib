...
---+ package TotalCmd::CompareResult
...
use Exception::Class (
    'Exception',
    'TotalCmd::CompareResult::Exception' =>
        { isa => 'Exception' },
);
...
Returns a new =TotalCmd::CompareResult= object for compare results
...
sub read {
    ...
    open (my $fh, '<', $self->{path})
        || throw TotalCmd::CompareResult::Exception("$!");
    ...
    while (<$fh>) {
        ...
        if (/(.*)\\$/) {
            ...
            throw TotalCmd::CompareResult::Exception("Unmatched: $_");
        }
    }
    close $fh;
