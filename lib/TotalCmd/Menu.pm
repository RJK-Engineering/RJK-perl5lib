...
---+++ new(title => $title, items => \@items) -> TotalCmd::Menu
Returns a new =TotalCmd::Menu= object.
...
Throws TotalCmd::Exception if multiple items found.
...
sub findItem {
    ...
    return if ! @items;
    throw TotalCmd::Exception("Found " . scalar @items . " items") if @items > 1;
    return $items[0];
}
...
Throws TotalCmd::Exception if multiple submenus found.
...
sub findSubmenu {
    ...
    return if ! @submenus;
    throw TotalCmd::Exception("Found " . scalar @submenus . " submenus") if @submenus > 1;
    return $submenus[0];
}
