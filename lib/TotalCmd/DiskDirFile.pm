...
---+ package TotalCmd::DiskDirFile

=cut

package TotalCmd::DiskDirFile;
use parent 'File::File';
...
Returns a new =TotalCmd::DiskDirFile= object.
...
sub _createFile {
    my ($self, $file) = @_;
    my $f = new File::File("$self->{root}\\$file->[0]");
    $f->{size} = $file->[1];
...
sub _createDir {
    my ($self, $dir) = @_;
    my $f = new File::File("$self->{root}\\$dir->[0]");
