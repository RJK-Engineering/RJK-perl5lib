...
use Exception::Class (
    'Exception',
    'TotalCmd::Log::Exception' =>
        { isa => 'Exception' },
    'TotalCmd::Log::CorruptLineException' =>
        { isa => 'Exception',
          fields => [qw(line)] },
);


use TotalCmd::Log::Entry;
...
---+++ traverse($file, $entryCallback, $corruptLineCallback)
   ...
   * throws TotalCmd::Log::Exception
   * throws TotalCmd::Log::CorruptLineException, unless
...
sub traverse {
    ...
    open(my $fh, '<', $file)
        || throw TotalCmd::Log::Exception("$!");

    $entryCallback && ref $entryCallback && ref $entryCallback eq "CODE"
        || throw TotalCmd::Log::Exception("No callback");

    if ($corruptLineCallback) {
        ref $corruptLineCallback && ref $corruptLineCallback eq "CODE"
            or throw TotalCmd::Log::Exception("Invalid corrupt line callback");
    } else {
        $corruptLineCallback = sub {
            my $line = shift;
            throw TotalCmd::Log::CorruptLineException(
    ...
    while (<$fh>) {
        ...
        my $entry = new TotalCmd::Log::Entry;
