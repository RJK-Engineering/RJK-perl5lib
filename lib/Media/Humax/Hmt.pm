package Media::Humax::Hmt;

use strict;

use Exception::Class (
    'Exception',
    'Media::Humax::HmtException' =>
        { isa => 'Exception' },
);
...
sub GetInfo {
    ...
    open ($fh, $file) or throw Media::Humax::HmtException("$!");
    binmode $fh;
    ...
    if ($bookmarkCount > 31) {
        throw Media::Humax::HmtException("Bookmark count exceeds 31");
    }
    ...
    if ($eventCount > 31) {
        throw Media::Humax::HmtException("Event count exceeds 31");
    }
...
#~ printf "! %x\n", $offset;
    ...
    if ($size1 && $size1 == $size2) {
        ...
        if ($blockCount > 31) {
            throw Media::Humax::HmtException("Block count exceeds 31");
        }
